...
func (m *Mailer) RenderTemplate(templateFile string, data interface{}) (string, error) {
	// Use the ParseFS() method to parse the required template file from the embedded
	// file system.
	tmpl, err := template.New("email").ParseFS(templateFS, "templates/"+templateFile)
	if err != nil {
	...
	// Execute the template with the given data and return the result as a string.
